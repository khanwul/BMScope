...
await request(`/api/charts?sha256=${hash}`)
assert.deepEqual(searchParams, [hash])
assert.equal((await request('/api/charts?sha256=nope')).status, 400)
...
console.log('ok — 저장 채보 API (목록 · 원문 · 경로 제한 · IR 프록시)')
