...
- **IR 탭** — 처음 열 때 BMS-IR·MinIR·Bokutachi·구 LR2IR을 최대 EX가 같은 기록끼리 비교하고, 개인 PB 격차와 분석 태그를 합친 반복 연습 구간·PB 성장·라이벌·다음 채보를 제안한다. 추천 채보가 서버에 저장돼 있으면 바로 불러오고, Mocha/STELLAVERSE로도 이동한다.
...
기존 DB에서 추천 채보 불러오기를 사용하려면 한 번 다시 import해 SHA-256을 채운다.
